...
class BuyMovieButton extends Component {
    ...
    componentDidMount() {
        if (this.props.auth.isAuthenticated) {
            axios.post('/api/payments/user_owns_movie/', {
    ...
    render() {
        if (!this.props.auth.isAuthenticated) {
            return <button disabled="disabled" className="btn btn-primary">Sign In to Buy</button>
        }
        let innerComponent;
        if (this.state.owned === "no") {
            innerComponent = (
                <Link to={`/payment/checkout/${this.props.movieId}`}
            ...
            innerComponent = (
                <button disabled="disabled" className="btn btn-primary">Owned</button>
            );
        }
        else {
