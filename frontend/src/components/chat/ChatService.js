class ChatService {
    ...
    connect(userId, authToken) {
        ...
        let url = `ws://localhost:8000/ws/chat/user/${userId}/?token=${authToken}`;
    ...
    _onAllMessagesReceived(messages) {
        if (this.onAllMessagesReceived)
            this.onAllMessagesReceived(messages);
    }
    ...
    _onUserInfoReceived(info) {
        if (this.onUserInfoReceived)
            this.onUserInfoReceived(info);
    }
