...
export const getWishList = () => (dispatch) => {
  ...
  axios
    ...
    .catch(({ response }) => {
      dispatch(
        createErrorResponseAction(types.MOVIE_WISH_LIST_GET_RESPONSE, response)
      );
    });
};
...
export const wishListAddMovie = (movieId, userId) => (dispatch) => {
  ...
  axios
    .post(WISH_LIST_URL, { movie: movieId, user: userId })
    .then((response) => {
      dispatch({
    ...
    .catch(({ response }) => {
      dispatch(
        createErrorResponseAction(types.MOVIE_WISH_LIST_ADD_RESPONSE, response)
      );
    });
};
...
export const wishListRemoveMovie = (wishListId) => (dispatch) => {
  ...
  axios
    .delete(`${WISH_LIST_URL}${wishListId}/`)
    .then((response) => {
      dispatch({
    ...
    .catch(({ response }) => {
      dispatch(
        createErrorResponseAction(
          types.MOVIE_WISH_LIST_REMOVE_RESPONSE,
          response
        )
      );
    });
